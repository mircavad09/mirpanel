/* =========================================================
   app.js — Mirpanel (FINAL v15)
   - Kataloq / filter / search
   - Modal + plan seçimi + WhatsApp yönləndirmə
   - InfoBox (modalın içində)
   - Music player (header)
   - iPhone modal scroll lock fix
   - ✅ Spring Creator Paket (Kampaniya)
   - ✅ Sayta girən kimi Spring POPUP (şəkil + Ad+Kod+Gmail -> WhatsApp)
   - ✅ Sayta girən kimi Spring BANNER (şəkil + CTA)
  ...
  products: [
    {
      id: "spring_creator",
      category: "bundle",
      title: "Spring Creator Paket",
      variant: "Kampaniya",
      image: "assets/spring.png",
      badge: "Paket",
      desc: "CapCut Pro + Netflix Şəxsi + ChatGPT Plus",
      note: "Paket: CapCut Pro + Netflix Şəxsi + ChatGPT Plus. Ad + 4 rəqəmli kod + Gmail yaz → WhatsApp-a göndər.",
      currency: "₼",
      plans: [{ months: 1, label: "1 aylıq Paket", price: 17.99 }],
    },
...
const INFO_TEXTS = {
  spring_creator: {
    title: "🌸 Spring Creator Paket (Kampaniya)",
    lines: [
         "✅ Paket: CapCut Pro + Netflix Şəxsi + ChatGPT Plus",
         "🔥 Kampaniya: 17.99₼ (1 aylıq)",
         "📌 Netflix üçün: Ad + 4 rəqəmli kod",
          "📧 ChatGPT üçün: Gmail (dəvət göndərilir)",
    ],
  },
  ...
  canva: {
    title: "🎨 Canva Premium",
    lines: [ "✅ Premium dizaynlar, şablonlar və elementlər açıq olur",
    "📌 Hesabınıza dəvət göndərilir, qəbul edib Premium-a keçirsiniz", ],
  },

  duolingo: {
    title: "📚 Duolingo Super",
    lines: [ "✅ Reklamsız istifadə",
      "✅ Limitsiz can (hearts)",
  ...
  adobecc: {
    title: "🎨 Adobe Creative Cloud",
    lines: [ "🎨 Dizayn | Video | Foto | Web — hamısı bir yerdə!",
...
"📌 Hesab biz tərəfdən hazır şəkildə təqdim edilir", ],
  ...
  prime: {
    title: "Amazon Prime Video",
    lines: [ "✨ Tam fərdi hesab — sürətli, təhlükəsiz və limitsiz izləmə imkanı!",
"🧩 Otaq yalnız sənə məxsusdur — paylaşım yoxdur.", ],
  },

  spotify: {
    title: "Spotify Premium",
    lines: [ "🎶 Limitsiz musiqi — reklamsız dinləmə və offline istifadə!",
...
}; // ✅ INFO_TEXTS burada bağlanmalıdır
...
/* =========================
   SPRING BANNER (Landing)
   - Hero bölməsinin altına inject edir
   ========================= */
function ensureSpringBanner() {
}

/* =========================
   SPRING POPUP
   ========================= */
function ensureSpringEnd() {
  const KEY = "mirpanel_spring_end_v1";
  const now = Date.now();
...
function openSpringPopup() {
  if (document.getElementById("springPopup")) return;

  const p = DATA.products.find((x) => x.id === "spring_creator");
  if (!p) return;

  const end = ensureSpringEnd();

  const wrap = document.createElement("div");
  wrap.id = "springPopup";
  wrap.className = "springPopup";

  wrap.innerHTML = `
    <div class="springCard" role="dialog" aria-modal="true">
      <button class="springX" type="button" aria-label="Bağla">✕</button>

      <div class="springTop">
        <div class="springVisual">
          <img class="springPoster" src="${esc(p.image)}" alt="Spring Creator">
        </div>
        <div class="springRight">
          <div class="springBadge">🌸 Kampaniya</div>
          <div class="springTitle">Spring Creator Paket</div>
          <div class="springSub">CapCut Pro + Netflix Şəxsi + ChatGPT Plus</div>
        </div>
      </div>

      <div class="springTimer">
        <div class="springTimerT">🔥 Kampaniya bitməsinə</div>
        <div class="springTimerV" id="springTimerV">yüklənir...</div>
      </div>

      <div class="springPrice">
        <div class="springOld">Normal: 23₼</div>
        <div class="springNew">İndi: 17.99₼ (1 aylıq)</div>
      </div>

      <div class="springInfo">
        <div class="springLine">✅ Netflix üçün: Ad + 4 rəqəmli kod</div>
        <div class="springLine">✅ ChatGPT üçün: Gmail (dəvət göndərilir)</div>
        <div class="springLine">✅ Hesablar hazır şəkildə təqdim olunur</div>
      </div>

      <div class="springForm">
        <div class="springGrid">
          <div>
            <div class="springLabel">Ad (Netflix üçün)</div>
            <input id="sp_name" class="springInput" placeholder="Məs: Mələk" />
          </div>
          <div>
            <div class="springLabel">4 rəqəmli kod</div>
            <input id="sp_code" class="springInput" inputmode="numeric" maxlength="4" placeholder="Məs: 2255" />
          </div>
        </div>

        <div style="margin-top:10px">
          <div class="springLabel">Gmail (ChatGPT üçün)</div>
          <input id="sp_gmail" class="springInput" placeholder="misal@gmail.com" />
        </div>

        <button id="sp_send" class="springBtn" type="button">İndi sifariş et → WhatsApp</button>
        <div class="springSmall">X ilə bağlayıb kataloqa baxa bilərsiniz.</div>
      </div>
    </div>
  `;

  document.body.appendChild(wrap);
  document.documentElement.classList.add("springLock");
  document.body.classList.add("springLock");

  wrap.querySelector(".springX")?.addEventListener("click", () => {
    wrap.remove();
    document.documentElement.classList.remove("springLock");
    document.body.classList.remove("springLock");
  });

  const timerEl = document.getElementById("springTimerV");
  ...
  function tick() {
    ...
    const ss = s % 60;
    if (timerEl) timerEl.textContent = `${pad2(d)} gün : ${pad2(h)} saat : ${pad2(m)} dəq : ${pad2(ss)} san`;
  }

  tick();
  const int = setInterval(() => {
    if (!document.getElementById("springPopup")) return clearInterval(int);
    tick();
  }, 1000);

  document.getElementById("sp_send")?.addEventListener("click", () => {
    const name = (document.getElementById("sp_name")?.value || "").trim();
    const code = (document.getElementById("sp_code")?.value || "").trim();
    const gmail = (document.getElementById("sp_gmail")?.value || "").trim();

    if (!name) return alert("Ad yaz.");
    if (!/^\d{4}$/.test(code)) return alert("4 rəqəmli kod yaz.");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(gmail)) return alert("Düzgün Gmail yaz.");

    const text =
`Salam, Spring Creator Paket sifariş etmək istəyirəm.
Müddət: 1 aylıq
Qiymət: 17.99 ₼
...
function cardHTML(p, idx) {
  ...
  const isSpring = p.id === "spring_creator";

  return `
    <div class="card ${isSpring ? "springCard" : ""}" style="animation-delay:${Math.min(idx * 0.03, 0.25)}s">
      <div class="imgWrap">
        <img class="img" src="${esc(p.image)}" alt="" onerror="this.style.opacity='.2'">
        <div class="cornerPrice">${esc(showPrice)}</div>
      </div>

      <div class="pad ${isSpring ? "springPad" : ""}">
        ${isSpring ? `
          <div class="springRow">
            <div class="springTxt">
  ...
  if (!plans.length) {
    ...
    plans.forEach((pl) => {
      ...
      d.addEventListener("click", () => {
        ...
        if (p.id === "spring_creator") return showBundleForm(p, pl);
...
function showBundleForm(p, plan) {
  const { fullName, duration, priceText } = pack(p, plan);

  $("mForm").innerHTML = `
    <div class="mpForm">
      <div class="mpFormTitle">Paket məlumatları</div>

      <div class="mpGrid2">
        <div>
          <div class="mpLabel">Ad (Netflix üçün)</div>
          <input id="b_name" class="mpInput" placeholder="Məs: Mələk">
        </div>

        <div>
          <div class="mpLabel">4 rəqəmli kod (Netflix üçün)</div>
          <input id="b_code" class="mpInput" inputmode="numeric" maxlength="4" placeholder="Məs: 2255">
        </div>
      </div>

      <div style="margin-top:10px">
        <div class="mpLabel">Gmail (ChatGPT üçün)</div>
        <input id="b_gmail" class="mpInput" placeholder="misal@gmail.com">
      </div>
      ...
      <button id="b_send" type="button" class="mpBtn">WhatsApp-a göndər</button>
    </div>
  `;
  ...
  $("b_send").onclick = () => {
    ...
    const gmail = ($("b_gmail")?.value || "").trim();
