...
function loadData(){
  ...
  if (!parsed || !Array.isArray(parsed.products)) return structuredClone(DEFAULT_DATA);
  if (!parsed.phone) parsed.phone = DEFAULT_DATA.phone;
  return parsed;
}
...
function renderCards(){
  ...
  (DATA.products || []).forEach(p=>{
    ...
    card.innerHTML = `
      <div class="corner-price">${escapeHtml(p.cornerPrice || "")}</div>
      <img class="card-img big" src="${escapeHtml(p.img || "")}" alt="">
      <div class="content">
  ...
  applyFilters(); // yenidən render olanda filter də qalır
}
renderCards();
...
function renderPlans(plans){
  ...
  plans.forEach((p)=>{
    ...
    row.innerHTML = `
      <div class="plan-left">
        <div class="radio"></div>
        <div class="plan-title">${escapeHtml(p.label)}</div>
      </div>
...
/* =========================
   AUTO REFRESH (ADMIN -> MAIN)
   ========================= */

// 1) Eyni brauzerdə "localStorage dəyişdi" siqnalı gələndə
...
// 2) Eyni tabda da işləsin deyə BroadcastChannel
let bc = null;
try {
  bc = new BroadcastChannel("mirpanel_sync");
  bc.onmessage = (ev) => {
...
// 3) Son çarə: hər 2 saniyədə yoxla (problemi 0 eləyir)
