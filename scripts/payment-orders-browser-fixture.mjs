...
window.api=async(path,options={})=>{
 ...
 if(parsed.pathname==="/api/admin/payment-orders"){
  ...
  return {orders:rows.slice(start,start+pageSize),counts:{pending:state.pending.length,today:3,all:state.all.length,expiring:state.expiring.length},statistics:{count:statsRows.length,revenue:Number(statsRows.reduce((sum,item)=>sum+item.amount,0).toFixed(2)),cost:65,profit:50,profitMargin:43.48,missingCostCount:2,topProduct:statsRows.length?"CapCut Pro":"—",topProfitProduct:"CapCut Pro",products:[{title:"CapCut Pro",count:Math.ceil(statsRows.length/2),revenue:80,cost:40,profit:40,margin:50,missingCostCount:1}]},pagination:{page,pageSize,total,totalPages:Math.max(1,Math.ceil(total/pageSize))},filters:{products:[{id:"capcut",title:"CapCut Pro"},{id:"netflix",title:"Netflix Şəxsi"}],plans:["1 aylıq","3 aylıq"],methods:[{id:"00000000-0000-4000-8000-000000004419",label:"LeoBank •••• 4419"}]}};
 }
