...
// v6 adds card rotation; it must delegate to v5's locked financial snapshot operation.
assert.ok(store.includes('rpc("approve_payment_order_v6"'));
const activationMigration = read("supabase/migrations/202609010001_payment_method_activation_policy.sql");
const approvalWrapper = activationMigration.match(/create or replace function public\.approve_payment_order_v6\([\s\S]*?\n\$\$;/)?.[0];
assert.ok(approvalWrapper?.includes("v_result := public.approve_payment_order_v5(p_order_id, p_duration_months, p_actor);"));
assert.ok(approvalWrapper.includes("return v_result;"));
const usageMigration = read("supabase/migrations/202608150001_payment_usage_day_and_method_soft_delete.sql");
const approval = usageMigration.match(/create or replace function public\.approve_payment_order_v5\([\s\S]*?\n\$\$;/)?.[0];
assert.ok(approval?.includes("where id = p_order_id for update;"));
assert.ok(approval.includes("if v_order.status = 'approved' then"));
assert.ok(approval.includes("sale_price_snapshot = amount, cost_price_snapshot = v_cost, profit_snapshot = v_profit"));
assert.ok(approval.indexOf("'idempotent', true") < approval.indexOf("confirmed_count = confirmed_count + 1"));
