-- Restore only previous functions and their privileges, NEVER payment rows.
begin;
set local lock_timeout='5s';
select pg_advisory_xact_lock(714025001);
do $$ declare f record; g record; begin
 for f in select * from mirpanel_status_queue_backup_20260903.function_definitions loop
   execute f.definition;
   execute format('revoke execute on function %s from public,anon,authenticated,service_role',f.signature);
   for g in select * from mirpanel_status_queue_backup_20260903.function_grants where signature=f.signature loop
