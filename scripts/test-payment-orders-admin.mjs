...
assert.ok(store.includes("contactExpiringOrders"), "Toplu əməliyyat mövcud fərdi auditli contacted RPC-sini istifadə etməlidir");
assert.ok(store.includes('selection: filters.tab === "expiring"'), "Filtrdəki bütün bitən nəticələr serverdən seçilməlidir");
...
assert.ok(api.includes('/api/admin/payment-orders/batch-contacted'));
assert.ok(api.includes('(catalog.products || [])') && api.includes('product.active !== false'), "Məhsul filtri aktiv kataloqdan gəlməlidir");
...
assert.ok(admin.includes("paymentSelectAllFiltered") && admin.includes("confirmSelectedExpiringOrders"));
assert.ok(admin.includes("selectedOrderIds.clear()"), "Filtr dəyişəndə gizli seçim qalmamalıdır");
