...
export async function createTestDatabase() {
  ...
  const rewrite=s=>s.replace(/create role anon; create role authenticated; create role service_role bypassrls;/g,'')
    ...
    .replace(/mirpanel_status_queue_backup_20260903/g,`${schema}_status_backup`)
    .replace(/mirpanel_capacity_backup_20260906/g,`${schema}_capacity_backup`);
  const db={
    real:true,schema,
    ...
    close:async()=>{
      await client.unsafe(`drop schema if exists ${schema}_capacity_backup cascade; drop schema if exists ${schema}_status_backup cascade; drop schema if exists ${schema}_bank_slot_backup cascade; drop schema if exists ${storage} cascade; drop schema if exists ${schema} cascade;`);
