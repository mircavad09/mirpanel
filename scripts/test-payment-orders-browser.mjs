...
try {
  ...
  assert.match(await page.textContent("#paymentCurrentMonthReport"), /187\.73/);
  ...
  assert.match((await page.locator(".paymentOrderDay summary").allTextContents()).join(" "), /9 avqust 2026/);
  ...
  assert.equal(await page.locator("#paymentMonthlyReports").isHidden(), false);
  assert.match(await page.textContent("#paymentCurrentMonthReport"), /Tamamlanmış sifariş/);
  await page.selectOption("#paymentOrderPeriod", "custom");
  assert.equal(await page.locator(".paymentCustomDate.isActive").count(), 2);
  assert.equal(await page.locator("#paymentOrderFiltersApply").isDisabled(), true);
  await page.fill("#paymentOrderDateFrom", "2026-08-18");
  await page.fill("#paymentOrderDateTo", "2026-08-10");
  assert.match(await page.textContent("#paymentOrderDateError"), /böyük ola bilməz/);
  await page.fill("#paymentOrderDateTo", "2026-08-18");
  assert.equal(await page.locator("#paymentOrderFiltersApply").isDisabled(), false);
  await page.evaluate(() => {
    const form = document.getElementById("paymentOrderFilters"); const search = document.getElementById("paymentOrderSearch");
    document.getElementById("paymentOrderPeriod").value = "all";
    search.value = "111"; form.requestSubmit();
    search.value = "222"; form.requestSubmit();
  });
  await page.waitForFunction(() => document.getElementById("paymentCurrentMonthReport")?.textContent.includes("222.00"));
