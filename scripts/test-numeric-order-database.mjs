...
try {
  ...
  check((await db.query("select * from payment_orders")).rows, before);
  await db.exec(migration);
  check((await db.query("select * from payment_orders")).rows, before);
  ...
  await db.exec(migration);
  check((await db.query("select last_number::int as n from payment_order_number_counter")).rows[0].n, 992);
  check((await db.query("select * from payment_orders where order_code='MP-ABC123'")).rows, before);
  ...
  console.log(JSON.stringify({ok:true, checks, engine:"isolated PostgreSQL/WASM", firstId:"971", rollback:true, repeatedRequests:40, duplicateIds:0, legacyRowsUnchanged:true, migrationRerun:true, note:"Single-connection engine; not a live multi-connection load test"}));
